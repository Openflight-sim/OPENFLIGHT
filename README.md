...
*Current Date: Sunday, May 4, 2025 at 9:39 PM CEST (Madrid, Spain)*
...
* **Git:** You need Git installed to download (clone) the repository. If you don't have it, download and install it from [git-scm.com](https://git-scm.com/downloads).
...
2.  **Navigate to your Development Directory:** Choose a location on your computer where you want to store the project files. Use the `cd` command to navigate there.
    ```bash
    ...
    cd dev
    ```
3.  **Clone the Repository:** Use `git clone` followed by the repository URL (replace the placeholder URL with the actual one for OpenFlight):
    ```bash
    git clone [https://github.com/YourUsername/OpenFlight.git](https://github.com/YourUsername/OpenFlight.git)
    ```
...
1.  **Download Julia:** Go to the official Julia language download page: [julialang.org/downloads/](https://julialang.org/downloads/)
2.  **Choose Version:** Download the **current stable release** (e.g., v1.10.x or later as of May 2025). Avoid alpha or beta versions unless you have a specific reason.
3.  **Install Julia:** Run the installer you downloaded.
...
4.  **Verify Installation:** Open a *new* terminal/command prompt window and type:
    ```bash
    julia --version
    ```
...
1.  **Download VS Code:** Go to the official VS Code website: [code.visualstudio.com/](https://code.visualstudio.com/)
2.  **Install VS Code:** Download the installer for your operating system and run it. Follow the on-screen instructions.
...
1.  **Launch VS Code.**
2.  **Open the Extensions View:** Click the Extensions icon in the Activity Bar on the side of the window (it looks like square blocks), or press `Ctrl+Shift+X`.
...
2.  **Verify Julia Path (Usually Automatic):**
...
2.  **Run the Main Julia Script:** Type the following command in the terminal and press Enter:
    ```bash
    julia OpenFlight.jl
    ```
...
5.  **Check the Browser:** The OpenFlight simulator frontend should load in your browser. You might need to wait a moment for the WebSocket connection to establish between the frontend and the backend.
